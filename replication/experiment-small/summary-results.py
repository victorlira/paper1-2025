...
def generate_report_reffilter():
    # 1. Load ground truth from LOI.csv
    truth = {}
    try:
        with open('LOI.csv', newline='', encoding='utf-8') as f:
            reader = csv.DictReader(f)
            for row in reader:
                idx = int(row['id'])
                truth[idx] = (row['interference'].strip() == '1')
    except FileNotFoundError:
        sys.stderr.write("Error: LOI.csv not found.\n")
        sys.exit(1)

    # 2. Load predictions from summary_experiment.csv
    predictions = {}
    try:
        with open('summary_experiment.csv', newline='', encoding='utf-8') as f:
            reader = csv.DictReader(f, delimiter=';')
            for row in reader:
                idx = int(row['id'])
                reported = int(row.get('reportedInterferences', '0'))
                is_ref = int(row.get('isRefactoring', '0'))
                # tool predicts True iff reportedInterferences==1 and isRefactoring==0
                predictions[idx] = (reported == 1 and is_ref == 0)
    except FileNotFoundError:
        sys.stderr.write("Error: summary_experiment.csv not found.\n")
        sys.exit(1)

    # 3. Compute confusion matrix
    TP = FP = FN = TN = 0
    for idx, actual in truth.items():
        pred = predictions.get(idx, False)
        if actual and pred:
            TP += 1
    ...
    # 5. Write results to results_resume.txt
    with open('results_resume_SA_RefFilter.txt', 'w', encoding='utf-8') as out:
        ...
        out.write("Metrics:\n")
        out.write(f"  Precision: {precision:.3f}\n")
        out.write(f"  Recall   : {recall:.3f}\n")
        out.write(f"  F1 Score : {f1_score:.3f}\n")
        out.write(f"  Accuracy : {accuracy:.3f}\n")
   
def generate_report_sa():
    # 1. Load ground truth from LOI.csv
    truth = {}
    try:
        with open('LOI.csv', newline='', encoding='utf-8') as f:
    ...
    try:
        with open('summary_experiment.csv', newline='', encoding='utf-8') as f:
            ...
            for row in reader:
                idx = int(row['id'])
                reported = int(row.get('reportedInterferences', '0'))
                # tool predicts True iff reportedInterferences==1
                predictions[idx] = (reported == 1)
    except FileNotFoundError:
        sys.stderr.write("Error: summary_experiment.csv not found.\n")
        sys.exit(1)
    ...
    # 5. Write results to results_resume.txt
    with open('results_resume_SA.txt', 'w', encoding='utf-8') as out:
        out.write("Confusion Matrix:\n")
        out.write(f"  True Positives : {TP}\n")
        out.write(f"  False Positives: {FP}\n")
        out.write(f"  False Negatives: {FN}\n")
