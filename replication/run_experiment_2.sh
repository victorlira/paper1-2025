...
DATASET_ZIP_URL="https://anonymous.4open.science/api/repo/ref-dataset-974E/zip"
./get-dataset.sh "$DATASET_ZIP_URL"

chmod +x output/files/prepare-dataset.sh
bash output/files/prepare-dataset.sh

INPUT_CSV="experiment-2/results-with-build-information.csv"
OUTPUT_DIR="output/data"
OUTPUT_CSV="$OUTPUT_DIR/results-with-build-information.csv"
RESULTS_DIR="experiment-2"
...
while IFS= read -r line; do
  ...
  "$JDK21_BIN" -jar dependencies/refactoring-analyser-1.0-SNAPSHOT-jar-with-dependencies.jar \
     "$SCRIPT_DIR/experiment-2" "$counter"
